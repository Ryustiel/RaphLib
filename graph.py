...
def conversation_node(state: State):
    ...
    return {"msg": state.CHAT, "out": [state.CHAT.messages[-1].content]}
...
def check_node(state: State):
    ...
    if response.success:
        ...
        state.CHAT
        return {
            "msg": state.CHAT.appendleft("temp", 
                """
...
def quit_node(state: State):
    state.CHAT.appendleft("temp", 
        ...
        """)
    message = (state.CHAT | LLM).invoke({}).content
    return {
        "msg": state.CHAT.remove(["temp"]).append("ai", message), 
        "out": [message]
    }
...
BUILDER.add_conditional_edges("Check", lambda state: state.next, {"quit": "Quit", "chat": "Chat"})
